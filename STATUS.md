...
- **Mobile web (Expo `--web` build) couldn't reach the API at all** — every fetch to `lucylabs.app` failed with a CORS error from the browser (native iOS/Android never hits this, CORS is browser-only). Fixed with an `Access-Control-Allow-Origin` header on `/api/:path*` in `web/next.config.ts`.
...
**Immediate next step**
- Re-pick `PRESET_VOICES` reference clips for `voice_meditation` / `voice_sales` in `scripts/lucy_tts_engine.py` (clip numbering may have changed since retrain; note the module moved out of `06_inference_server.py` in the Serverless refactor — that file is now just a thin FastAPI wrapper for local/admin testing), restart inference, ear-test Michelle + Robbo. **Confirmed still genuinely open as of 2026-09-09's cleanup pass** — a full code audit found `MAX_CHUNK_WORDS_BY_VOICE["voice_meditation"] = 14` and `PITCH_JITTER_BY_VOICE["voice_sales"] = 0.4` both still at their original bandage/stopgap values, unchanged since the retrain, with no commit or doc evidence the ear-test/re-pick step ever happened. Two fresh samples were generated this session (not saved anywhere persistent - sent directly to the user) so there's something current to listen to, but the actual judgment call requires a human ear, not something this review could resolve on its own.
...
- **`auth-accounts-history` branch is built, tested, and unmerged** — full email-magic-link + Google OAuth sign-in, Stripe Billing Portal self-serve billing, and short-retention generation history. Deliberately not merged to `main` without the user's own review first (it changes account/billing surface on a live paid product). See the branch's own commit for scope.
