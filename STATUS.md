...
**Product / brand**
- Product renamed to Lucy / Lucy Labs / Astryks Group throughout the customer-facing site and app (codebase stays "Sloane" internally, by design)
...
- Evaluated three candidate open-source models: OmniTalker (no public code/weights — genuine dead end, confirmed), Hallo3 (found to have weak identity preservation in benchmarks — rejected), **EchoMimicV3** (chosen: Apache-2.0 licensed weights, smaller 1.3B model, no identity red flags)
- Got EchoMimicV3 actually generating video end-to-end on the GPU pod, after fixing six separate environment issues (disk quota, missing dependency, and two rounds of version-compatibility pinning between torch/diffusers/transformers)
...
**Risk management**
- All four open-source projects we depend on (Chatterbox, the Chatterbox fine-tuning toolkit, Hallo3, EchoMimicV3) have their **source code** copied into our own git history under `vendor/`, so we're not exposed if any of them go private, get deleted, or relicense. (Model *weights* — multi-gigabyte files — can't live in git; they stay on the RunPod persistent storage volume, but the exact download commands to regenerate them are documented in `PROJECT_CONTEXT.md`.)
...
- The real fix — fine-tuning EchoMimicV3 on actual video of the person — needs real source video footage, same blocker as above
...
The user's Mac (which has more instructor video footage) is a different device from this Windows machine, so Claude Code running here can't reach it directly — there's no remote/network access between them. To get that footage in: AirDrop/cloud drive (iCloud Drive, Google Drive, OneDrive) to a folder synced to this Windows machine, or a direct USB/cable transfer, then point Claude at the resulting local folder.
