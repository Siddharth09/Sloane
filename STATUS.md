...
- **Fal.ai research, 2026-09-08 (separate session, reviewed and merged by this one)**: a parallel session smoke-tested real hosted video APIs (fal.ai, proxying Kling Avatar, Veo 3.1, Grok Imagine, MiniMax, Seedance) against Vicky's (`art_instructor`) actual photo/likeness. Findings and exact fal COGS are in [`docs/fal-video-research/README.md`](./docs/fal-video-research/README.md) — worth reading in full. **Only the README made it into the repo** — the actual demo videos/stills/audio and the prompts file referenced throughout it still have **not** transferred to this machine (a ~63MB tarball on the other session's machine never landed here, despite repeated asks) - the "preferred demo" / asset-index links in that README remain pending, not present, and nothing can be verified, re-encoded, or used as a website trailer until the real files exist somewhere this session can read them.
- **Data-sharing decision (2026-09-08)**: sending a real person's likeness/voice to third-party vendors (Kling/Veo/Grok/etc. via fal.ai) is a materially different exposure than this project's self-hosted consent posture (`PROJECT_CONTEXT.md` Sec 3). **Decision: proceed with a user-facing disclaimer rather than blocking on new consent infrastructure** - if/when this ships, the video UI must clearly disclose that likeness/voice data is sent to third-party AI vendors for processing, distinct from the fully self-hosted audio pipeline. Disclaimer copy itself not yet written.
- **Pricing/margin review (2026-09-08)**: checked the proposed credit math against worst-case COGS (a subscriber spending their entire credit balance on the single most expensive option). Confirmed **safe**: Plus $5/mo @ 30 credits (57.4% worst-case margin after Stripe fees), Pro $12/mo @ 90 credits (51.5%), or keeping Pro at $9/mo but cutting to 60 credits (56.3%) - any of these three are fine. **Confirmed unsafe: keeping Pro at $9/mo with 90 credits (only 37.6% worst-case margin)** - too thin once you account for things this simple model doesn't (occasional fal charges on failed calls, Stripe disputes/refunds, support overhead, the shared audio-GPU fixed cost still needing to be covered by the subscriber base). **Recommendation: Pro $12/90 or Pro $9/60, not $9/90.** This only holds in practice if credit debiting is enforced server-side, preflight, only on confirmed fal success - never client-trusted or optimistic. Still just a reviewed proposal - no Stripe prices or code changed.
