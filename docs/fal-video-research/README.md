...
**What this settles vs. what's still open**: settles that the core technical mechanism (generate once, store the image, reuse it for every future scene) genuinely works, with real output as proof, not just the theory from earlier in the day. Still fully open: the `ai_actors` table, private storage, the similarity safeguard, and the real generation endpoint - this was API-level proof, not a shipped feature.

**Real visual-quality verdict from actually watching the clips (2026-09-10)**: **Veo is the most realistic of the three. Seedance looks cartoonish, not good. Kling is okay.** This is real ear/eye feedback, not a guess, and it changes the engine-routing recommendation from earlier the same day: that recommendation picked Seedance as the fallback for "stylized actor look" specifically *because* it couldn't do hyper-realism (the content-policy block) - but if Seedance's actual rendering quality is subpar even for its own stylized lane, it's a weaker default than assumed, not just a hyper-realism-only limitation. **Revised recommendation**: Veo first choice for realism-critical shots (already the case for Cinematic; extend to Ads-mode silent-action shots too), Kling for talking-head/lip-sync and for anything needing guaranteed character consistency across separate generations (its real strength, independent of raw visual polish), Seedance narrowed to only its one truly unique capability - the `@Image1`/`@Video1` upload-a-video motion-transfer trick - not used as a general-purpose "stylized" fallback engine.

**A real limit on "exclusive" surfaced by direct questioning, not discovered by testing - worth recording here since it changes how this should ever be marketed**: once a character's video exists anywhere (delivered to a customer, posted publicly), *anyone* can extract a still frame from it (exactly the `ffmpeg` step used above) and feed that image into Kling/Veo/Seedance/anything else themselves - none of these engines have any concept of "this face belongs to Lucy Labs customer X," they're stateless and will animate whatever image they're handed. So "exclusive" **cannot be a technical guarantee** once the asset has been delivered or published - it has to be a **contractual/legal** one (licensing terms restricting off-platform reuse, the same mechanism real spokesperson-exclusivity deals already use, since nothing stops someone screenshotting a real actor off a billboard either), reinforced by not handing customers a clean ideal reference still (deliver finished video only - compressed/motion-blurred, real friction against extraction even though not a hard wall) and optionally provenance/watermark metadata for enforcement if misuse is found. This limitation is not specific to Lucy Labs - Arcads' identical "you'll be the only one able to use this" claim has the same scope (their own platform only, not the pixels themselves). Any future product copy for this feature needs to frame exclusivity this way, not as a technical impossibility.
